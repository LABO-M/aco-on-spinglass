...
@threads for sample_id in 1:num_samples
    ...
    for i in 1:iter
        ...
        for new_spins in 2:n
            external_effect = calculate_external_effect(determined_spins, J, new_spins) ./ Int(new_spins)
...
filename_m_mean = @sprintf("beta%.1e_sample%.1e_n2^%d_tau%.1e_modified_m_mean.csv", beta, num_samples, exp_n, tau)
full_path_m_mean = joinpath(dir_path, filename_m_mean)
CSV.write(full_path_m_mean, df_m_mean)
filename_m_var = @sprintf("beta%.1e_sample%.1e_n2^%d_tau%.1e_modified_m_var.csv", beta, num_samples, exp_n, tau)
