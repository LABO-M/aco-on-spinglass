...
function sampling(power, tau, beta, start_alpha, start_seed, iter, sample, magnetic, interaction)
    # --- mの時系列格納用 ---
    ...
    @threads for sample_id in 1:sample
        ...
        for i in 1:iter
            ...
            for new_spins in 2:n
                external_effect = -magnetic + calculate_external_effect(determined_spins, J, new_spins) ./ Int(new_spins)
            ...
            push!(spins_mean, mean(determined_spins))

            alpha += alpha_inc
        end

        m_mean_series_all[sample_id] = m_mean_series
        spins_mean_series_all[sample_id] = spins_mean
    ...
    dir_path = "/home/mori-lab/shimizu/aco/data/ising/annealing/seed$(start_seed)/iter$(iter_str)/asymmetric"
    mkpath(dir_path)

    # --- ファイル名と保存 ---
    exp_n = power

    filename_m_mean = @sprintf("beta%.1e_sample%.1e_n2^%d_tau%.1e_modified_m_mean.csv", beta, sample, exp_n, tau)
    full_path_m_mean = joinpath(dir_path, filename_m_mean)
    CSV.write(full_path_m_mean, df_m_mean)
    filename_spins_mean = @sprintf("beta%.1e_sample%.1e_n2^%d_tau%.1e_modified_spins_mean.csv", beta, sample, exp_n, tau)
