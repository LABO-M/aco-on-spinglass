...
# ============================================
#  2フェーズ版サンプリング
#   - フェーズ1: alpha=0.0 で iters_burnin 回
#   - フェーズ2: alpha=alpha_fixed で iters_fixed 回
# 保存する列は z_mean, spin_mean のみ
# それぞれ別CSVに出力
# ============================================
function simulate_two_phase(power::Int, tau::Float64, beta::Float64, seed::Int,
                            magnetic::Float64, interaction::Float64;
                            alpha_fixed::Float64=0.5,
                            iters_burnin::Int=100_000, iters_fixed::Int=100_000)
    ...
    # 1ステップ分進めて (z_mean, spin_mean) を返す内部関数
    function one_step!(α::Float64)
        determined_spins = Int[]
        # 1..n の順に決定（最初のスピンもフェロモン参照）
        for new_index in 1:n
            if new_index == 1
            ...
            prob = decision_probabilities(st1, st0, new_index, α, beta, external_effect)
            new_spin = 2 * (rand(rng) < prob) - 1
            push!(determined_spins, new_spin)
        end
        ...
        zvec = st1 ./ (st1 .+ st0)
        z_mean = mean(zvec)
        spin_mean = mean(determined_spins)    # {-1,1} の平均
        return z_mean, spin_mean
    end
    ...
    # ===== フェーズ1: alpha=0.0 バーンイン =====
    z_list_burnin   = Float64[]
    spin_list_burnin= Float64[]
    for _ in 1:iters_burnin
        z_mean, spin_mean = one_step!(0.0)
        push!(z_list_burnin, z_mean)
        push!(spin_list_burnin, spin_mean)
    end
    df_burnin = DataFrame(z_mean=z_list_burnin, spin_mean=spin_list_burnin)
    filename_burnin = @sprintf("two_phase_burnin_alpha0_beta%.1e_n2^%d_tau%.1e.csv", beta, power, tau)
    full_burnin = joinpath(dir_path, filename_burnin)
    CSV.write(full_burnin, df_burnin)

    # ===== フェーズ2: alpha=alpha_fixed 固定 =====
    z_list_fixed   = Float64[]
    spin_list_fixed= Float64[]
    for _ in 1:iters_fixed
        z_mean, spin_mean = one_step!(alpha_fixed)
        push!(z_list_fixed, z_mean)
        push!(spin_list_fixed, spin_mean)
    end
    df_fixed = DataFrame(z_mean=z_list_fixed, spin_mean=spin_list_fixed)
    filename_fixed = @sprintf("two_phase_fixed_alpha%.3f_beta%.1e_n2^%d_tau%.1e.csv",
                              alpha_fixed, beta, power, tau)
    full_fixed = joinpath(dir_path, filename_fixed)
    CSV.write(full_fixed, df_fixed)

    return filename_burnin, filename_fixed
end

# 便利エイリアス：引数で直接ファイル名を受け取りたい場合
function sampling_two_phase(power::Int, tau::Float64, beta::Float64, seed::Int;
                            magnetic::Float64=0.0, interaction::Float64=0.0,
                            alpha_fixed::Float64=0.5,
                            iters_burnin::Int=100_000, iters_fixed::Int=100_000)
    return simulate_two_phase(power, tau, beta, seed;
                              magnetic=magnetic, interaction=interaction,
                              alpha_fixed=alpha_fixed,
                              iters_burnin=iters_burnin, iters_fixed=iters_fixed)
end
